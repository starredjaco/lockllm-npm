/**
 * Tests for OpenAI Wrapper with Advanced Headers
 */
...
describe.skip('OpenAI Wrapper with Headers', () => {
  ...
  it('should use LockLLM proxy base URL', () => {
    ...
    expect(client._config.baseURL).toBe('https://api.lockllm.com/v1/proxy/openai');
  });
  ...
  it('should preserve OpenAI SDK options', () => {
    const client = createOpenAI({
      apiKey: 'test_key',
    ...
    expect(client._config.organization).toBe('org-123');
  ...
  it('should create functional OpenAI client', async () => {
    const client = createOpenAI({
      apiKey: 'test_key',
      proxyOptions: {
        scanMode: 'normal',
